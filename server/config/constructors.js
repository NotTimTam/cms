...
export const constructWebmaster = async () => {
	...
	if (!user) {
		...
		user = new UserModel({
			name: "Webmaster",
			username: "webmaster",
			password: await bcrypt.hash(password, +process.env.SALT || 12),
			verified: false,
		...
		warn(
			`No webmaster user exists. One has been created with the credentials:\n\nUsername: ${user.username}\nPassword: ${password}\n\nLogin as this webmaster and change the password immediately.`
		);
